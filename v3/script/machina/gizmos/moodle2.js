 ...
 */
 ...
 (function( Machina ) {
 	...
 	var Moodle = Machina.Gizmos.moodle2 = function(gizmos) {
 	...
 	Moodle.fn = Moodle.prototype = {
 		...
 		view: function(element, options) {
			...
			this.update = function(element, options) {
				...
				(properties)? (properties.moodle2)? (properties = properties.moodle2):{}:{};
				...
				function compose(content, data) {
					...
					modal.style.cssText = modal.style.cssText.replace(/display: ?none;/,"visibility:hidden;" + css);
					centerModal();
 	...
 	Moodle.fn.controller = Machina.methods.moodle2 = function(target) {
